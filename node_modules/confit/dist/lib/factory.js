...
var loadJsonicSync = _interopRequire(require("load-jsonic-sync"));
...
var Factory = (function () {
    function Factory(_ref) {
        ...
        this.promise = _core.Promise.resolve({}).then(function (store) {
            ...
            return Handlers.resolveImport(loadJsonicSync(file), _this.basedir).then(function (data) {
                return Common.merge(data, store);
            });
        })).then(Factory.conditional(function (store) {
            var file = Path.join(_this.basedir, "" + store.env.env + ".json");
            return Handlers.resolveImport(loadJsonicSync(file), _this.basedir).then(function (data) {
                return Common.merge(loadJsonicSync(file), store);
            });
        })).then(function (store) {
